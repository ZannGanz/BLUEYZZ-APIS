...
var creator = 'BlueyZz' // ubah jadi nama lu
const listkey = ["72808","blueejelek","727362"]; // ubah apikey nya, tambah aja klo mau
...
router.get('/cecan/china', async (req, res, next) => {
       ...
       fetch(encodeURI(`https://raw.githubusercontent.com/BlueyZzofc/Asupan/main/cecan/china.json`))
...
router.get('/cecan/vietnam', async (req, res, next) => {
          var apikey = req.query.apikey
       	if(!apikey) return res.sendFile(__path + '/docs/403.html')
        if(listkey.includes(apikey)){
       fetch(encodeURI(`https://raw.githubusercontent.com/BlueyZzofc/Asupan/main/cecan/vietnam.json`))
        .then(response => response.json())
        .then(async data => {
        var result = data[Math.floor(Math.random() * data.length)];
        var buffer = result.url;
